...
function sc_echo_item_list($atts) {
	if(!is_admin()){
		...
		// アフィリエイトコードの設定
		if(!(empty($scsz_feed_setting -> affiliate_code) && strcmp(get_option('sc_affiliate_agree'), '1'))){
...
function sc_reed_feed($scsz_feed_setting){
	...
	if(!empty($scsz_feed_setting -> max_num)){
		$scsz_option_params = $scsz_option_params.esc_url($scsz_feed_setting -> max_num);
	} else {
	...
	if(!empty($scsz_feed_setting -> page)){
		$scsz_option_params = $scsz_option_params."&pg=".esc_url($scsz_feed_setting -> page);
	}

	// フレーズ
	if(!empty($scsz_feed_setting -> phrase)){
		$scsz_option_params = $scsz_option_params."&qs=".esc_url($scsz_feed_setting -> phrase);
	}

	// 部門ID
	if(!empty($scsz_feed_setting -> department)){
		$scsz_option_params = $scsz_option_params."&dp=".esc_url($scsz_feed_setting -> department);
	}
	...
	if($scsz_feed_setting -> popular_flg == 1){
		...
		if(!empty($scsz_feed_setting -> popular_days)){
			$scsz_option_params = $scsz_option_params."&sp=".esc_url($scsz_feed_setting -> popular_days);
		}
	}
	...
	if(!empty($bg_color)){
		$scsz_option_params = $scsz_option_params."&bg=".esc_url($bg_color);
	}
